...
import kotlinx.android.synthetic.main.sports_fragment.progressBar_loading
import kotlinx.coroutines.launch
import org.kodein.di.KodeinAware
import org.kodein.di.android.x.closestKodein
import org.kodein.di.generic.instance

class SportsFragment : Fragment(), KodeinAware, SwipeRefreshLayout.OnRefreshListener {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        refreshSportsItems.setOnRefreshListener(this)
        viewModel =
            ...
            value?.let { show ->
                when (show) {
                    getString(R.string.resource_forbidden) ->
                        textErrorSports.text = getString(R.string.resource_forbidden)
                    getString(R.string.server_not_found) ->
                        textErrorSports.text = getString(R.string.server_not_found)
                    getString(R.string.internal_server_error) ->
                        textErrorSports.text = getString(R.string.internal_server_error)
                    getString(R.string.bad_gateway) ->
                        textErrorSports.text = getString(R.string.bad_gateway)
                    getString(R.string.resource_removed) ->
                        textErrorSports.text = getString(R.string.resource_removed)
                    getString(R.string.removed_resource_found) ->
                        textErrorSports.text = getString(R.string.removed_resource_found)
                    getString(R.string.network_error) ->
                        textErrorSports.text = getString(R.string.network_error)
                    getString(R.string.no_internet_connection) ->
                        textErrorSports.text = getString(R.string.no_internet_connection)
                    getString(R.string.connection_timed_out) ->
                        textErrorSports.text = getString(R.string.connection_timed_out)
                }
            }
        })
    }
    ...
    private fun initRecyclerView(items: List<NewsItems>) {
        ...
        recyclerViewSports.apply {
            layoutManager = LinearLayoutManager(this@SportsFragment.context)
    ...
    override fun onRefresh() {
        bindUI()
        hideErrorTextView()
        refreshSportsItems.isRefreshing = false
    }

    private fun hideErrorTextView() {
        textErrorSports.visibility = View.GONE
    }
}