...
import kotlinx.android.synthetic.main.business_fragment.progressBar_loading
import kotlinx.coroutines.launch
import org.kodein.di.KodeinAware
import org.kodein.di.android.x.closestKodein
import org.kodein.di.generic.instance

class BusinessFragment : Fragment(), KodeinAware, SwipeRefreshLayout.OnRefreshListener {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        refreshBusinessItems.setOnRefreshListener(this)
        viewModel =
            ...
            value?.let { show ->
                when (show) {
                    getString(R.string.resource_forbidden) ->
                        textErrorBusiness.text = getString(R.string.resource_forbidden)
                    getString(R.string.server_not_found) ->
                        textErrorBusiness.text = getString(R.string.server_not_found)
                    getString(R.string.internal_server_error) ->
                        textErrorBusiness.text = getString(R.string.internal_server_error)
                    getString(R.string.bad_gateway) ->
                        textErrorBusiness.text = getString(R.string.bad_gateway)
                    getString(R.string.resource_removed) ->
                        textErrorBusiness.text = getString(R.string.resource_removed)
                    getString(R.string.removed_resource_found) ->
                        textErrorBusiness.text = getString(R.string.removed_resource_found)
                    getString(R.string.network_error) ->
                        textErrorBusiness.text = getString(R.string.network_error)
                    getString(R.string.no_internet_connection) ->
                        textErrorBusiness.text = getString(R.string.no_internet_connection)
                    getString(R.string.connection_timed_out) ->
                        textErrorBusiness.text = getString(R.string.connection_timed_out)
                }
            }
        })
    }
    ...
    override fun onRefresh() {
        bindUI()
        hideErrorTextView()
        refreshBusinessItems.isRefreshing = false
    }

    private fun hideErrorTextView() {
        textErrorBusiness.visibility = View.GONE
    }
}